...
def get_user_config() -> Dict[str, Any]:
    ...
    # Required fields
    config["project_name"] = (
        input("Project name (lowercase, no spaces): ").lower().replace(" ", "-")
    )
    config["project_name_title"] = (
        input(f"Project title [{config['project_name'].title()}]: ")
        or config['project_name'].title()
    )
    config["project_description"] = (
        input("Project description: ") or f"A Python backend service"
    )
    ...
    config["github_repo"] = (
        input(
            f"GitHub repo [{config['author_name'].lower().replace(' ', '-')}/{config['project_name']}]: "
        )
        or f"{config['author_name'].lower().replace(' ', '-')}/{config['project_name']}"
    )
    config["database_name"] = config["project_name"]
    config["database_user"] = config["project_name"]
    config["service_description"] = (
        f"{config['project_name_title']} - {config['project_description']}"
    )
...
def generate_project(target_dir: str = None) -> None:
    ...
    if target_dir is None:
        target_dir = (
            input(f"\nTarget directory [{user_config['project_name']}]: ")
            or user_config['project_name']
        )
